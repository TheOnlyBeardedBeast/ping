 ...
 * Used for Arduino projects, just #include "ADeque.h" and add this file via the IDE.
 *
 * Examples:
 *
 * ADeque<char> queue(10); // Max 10 chars in this queue
 * queue.push_back('H');
 * queue.push_back('e');
 * queue.back(); // 2
 * queue.push_back('l');
...
template <class T>
inline int ADeque<T>::count()
{
    return _count;
}

template <class T>
inline int ADeque<T>::front()
{
    return _front;
}

template <class T>
inline int ADeque<T>::back()
{
    return _back;
}

template <class T>
void ADeque<T>::push_front(const T &item)
{
    if (_count < _maxitems)
    { // Drops out when full
        _front--;
...
template <class T>
void ADeque<T>::push_back(const T &item)
{
    if (_count < _maxitems)
    { // Drops out when full
        _data[_back++] = item;
